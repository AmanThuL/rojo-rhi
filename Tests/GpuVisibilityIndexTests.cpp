...
TEST_CASE("indirect instance index includes first instance", "[gpu][visibility][instance-index]") {
    ...
    auto pipeline =
        (*device)->createGraphicsPipeline({.library = library->get(),
                                           ...
                                           .label = "rojorhi.test.visibility.instanceIndex"});
    INFO(errorOf(pipeline));
    REQUIRE(pipeline.has_value());
    auto target = makeProbeTarget(**device, "rojorhi.test.visibility.indexTarget");
    REQUIRE(target.has_value());
    const uint32_t indices[] = {0, 1, 2};
    auto indexBuffer = (*device)->createBuffer(
        {.size = sizeof(indices), .label = "rojorhi.test.visibility.indices"}, indices);
    ...
    for (uint32_t count : {1u, 4u}) {
        ...
        auto output = (*device)->createBuffer({.size = sizeof(initial),
                                               .storageWrite = true,
                                               .cpuReadback = true,
                                               .label = "rojorhi.test.visibility.observed"},
                                              initial.data());
        ...
        auto arguments = (*device)->createBuffer(
            {.size = bytes.size(), .label = "rojorhi.test.visibility.args"}, bytes.data());
        REQUIRE(arguments.has_value());
        auto& commands = (*device)->beginFrame();
        commands.beginRenderPass({.colorTarget = target->get(),
                                  .clear = true,
                                  .label = "rojorhi.test.visibility.indexPass"});
